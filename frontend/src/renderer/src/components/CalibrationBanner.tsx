...
export default function CalibrationBanner() {
  ...
  return (
    <div className="absolute top-3 left-1/2 -translate-x-1/2 z-30 flex items-center gap-3 pl-3 pr-2 py-2 rounded-xl border border-amber-500/60 bg-panel shadow-token max-w-[min(680px,90%)]">
      <Ruler size={16} className="text-amber-500 shrink-0" />
      <span className="text-xs text-fg">
        {needsScale && <b className="text-amber-500">Sin calibrar — las medidas saldrán en píxeles. </b>}
        {hint}
      </span>
      {needsScale && (
        <button onClick={() => setActiveTool('measure_calibrate')}
          className="text-xs px-2.5 py-1 rounded bg-fg text-toolbar hover:opacity-90 transition-opacity shrink-0">
          Calibrar
        </button>
      )}
