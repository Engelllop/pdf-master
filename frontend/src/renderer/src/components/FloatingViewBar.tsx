  ...
  return (
    <div className="absolute bottom-5 left-1/2 -translate-x-1/2 z-30 flex items-center gap-1 px-2 py-1 rounded-xl border border-border bg-panel shadow-token">
      ...
      <div className="relative">
        <button onClick={() => setZoomMenu((o) => !o)} className="flex items-center gap-0.5 px-1.5 py-0.5 rounded hover:bg-hover text-fg text-mini font-mono" title="Nivel de zoom">
        ...
        {zoomMenu && (
          <>
            ...
            <div className="absolute bottom-full left-0 mb-1 z-50 w-32 border border-border rounded-token shadow-token py-1 bg-panel">
              ...
              <button onClick={() => { applyFit('fit-width'); setZoomMenu(false) }} className="w-full text-left px-3 py-1 text-mini text-fg hover:bg-hover">Ajustar al ancho</button>
              <button onClick={() => { applyFit('fit-page'); setZoomMenu(false) }} className="w-full text-left px-3 py-1 text-mini text-fg hover:bg-hover">Ajustar página</button>
            </div>
          </>
        )}
      </div>
      ...
      <span className="text-mini font-mono text-fg tabular-nums px-1">{activeDoc.currentPage + 1} / {activeDoc.page_count}</span>
