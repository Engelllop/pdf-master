...
export default function StampSignatureManager({ onClose }: { onClose: () => void }) {
  ...
  const tabBtn = (id: typeof tab, label: string, Icon: typeof StampIcon) => (
    <button onClick={() => setTab(id)}
      className={`flex items-center gap-1.5 px-3 py-1.5 text-[12px] rounded transition-colors ${
        tab === id ? 'bg-active text-accent' : 'text-muted hover:bg-hover hover:text-fg'
      }`}>
      <Icon size={13} /> {label}
    </button>
  )
  ...
  return (
    <div className="fixed inset-0 z-[93] flex items-center justify-center bg-black/50" onClick={onClose}>
      <div role="dialog" aria-modal="true" aria-label="Sellos y firmas" onClick={(e) => e.stopPropagation()}
        onKeyDown={(e) => { if (e.key === 'Escape') onClose() }}
        className="menu-pop w-[520px] max-w-[92vw] max-h-[84vh] flex flex-col rounded-lg border border-border shadow-2xl bg-panel text-fg">
        <div className="flex items-center gap-2 px-4 py-3 border-b border-border">
          <h2 className="text-sm font-semibold flex-1">Sellos y firmas</h2>
        ...
        {tab === 'stamps' ? (
          <div className="flex-1 overflow-y-auto p-3 space-y-3">
            <div>
              <p className="text-[10px] uppercase tracking-wider text-muted mb-1.5">De fábrica</p>
              <div className="flex flex-wrap gap-1.5">
                {BUILTIN_STAMPS.map((s) => (
                  <button key={s} onClick={() => useStamp(s, '#22c55e')}
                    className="px-2.5 py-1 rounded border border-border text-[11px] text-fg hover:bg-hover transition-colors">
                    {s}
                  </button>
                ))}
              </div>
            ...
            <div>
              <p className="text-[10px] uppercase tracking-wider text-muted mb-1.5">Mis sellos</p>
              {stamps.length === 0 && <p className="text-[11px] text-muted">Aún no has creado ninguno.</p>}
                ...
                {stamps.map((s) => (
                  <div key={s.id} className="group flex items-center gap-2 px-2 py-1.5 rounded border border-border">
                    ...
                    <button onClick={() => useStamp(renderStampText(s, annotationAuthor), s.color)}
                      className="flex-1 min-w-0 text-left text-[12px] text-fg truncate">
                      {renderStampText(s, annotationAuthor)}
                    </button>
                    <button onClick={() => { removeStamp(s.id); setStamps(loadStamps()) }} aria-label="Eliminar sello"
                      className="opacity-0 group-hover:opacity-100 p-1 rounded text-muted hover:text-red-500 transition-opacity">
                      <Trash2 size={12} />
                    </button>
                  </div>
                ))}
            ...
            <div className="border-t border-border pt-3 space-y-2">
              <p className="text-[10px] uppercase tracking-wider text-muted">Nuevo sello</p>
              <div className="flex items-center gap-2">
                <input value={text} onChange={(e) => setText(e.target.value)} placeholder="Texto del sello"
                  onKeyDown={(e) => { if (e.key === 'Enter') create() }}
                  className="flex-1 min-w-0 border border-border rounded px-2 py-1.5 text-[12px] bg-surface text-fg placeholder:text-muted focus:outline-none focus:border-accent" />
                ...
                <button onClick={create} disabled={!text.trim()}
                  className="flex items-center gap-1 px-3 py-1.5 rounded text-[12px] bg-fg text-toolbar disabled:opacity-40 hover:opacity-90 transition-opacity">
                  <Plus size={13} /> Crear
                </button>
              </div>
            ...
            {signatures.map((sig) => (
              <div key={sig.id} className="group flex items-center gap-2 px-2 py-1.5 rounded border border-border">
                ...
                <input defaultValue={sig.name}
                  ...
                  className="flex-1 min-w-0 bg-transparent text-[12px] text-fg focus:outline-none focus:border-b focus:border-accent" />
