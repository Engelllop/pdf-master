...
const api = {
  ...
  chooseFolder: (defaultPath?: string): Promise<string | null> => ipcRenderer.invoke('dialog:chooseFolder', defaultPath),
  ...
  onAiDone: (cb: (d: { requestId: string; truncated?: boolean }) => void) => {
    const h = (_e: unknown, d: { requestId: string; truncated?: boolean }) => cb(d)
